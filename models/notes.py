...
class NoteManager:
    ...
    def list_notes(self):
        for note in self.notes:
            print(f'{note.id}, {note.title}, \nПоследнее изменение в: {note.timestamp}')
    ...
    def delete_note(self, note_id):
        self.notes = [note for note in self.notes if note.id != note.id]
        self.save_notes()
        print('Заметка удалена')

    def export_to_csv(self, file_name="notes.csv"):
        with open(file_name, 'w', newline='') as fl:
            writer = csv.DictWriter(fl, fieldnames=["id", "title", "content", "timestamp"])
            writer.writeheader()
            for note in self.notes:
